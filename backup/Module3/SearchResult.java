...
public class SearchResult {
    ...
    public Boolean verifySizeChartExists(WebDriver driver) {
        ...
        try {
             ...
             */
            WebElement sizeChart = driver.findElement(By.xpath("//*[@id='root']/div/div/div[3]/div/div[2]/div[1]/div/div[1]/button"));
            if (sizeChart.getText().contains("SIZE CHART")) status = true;
            else status = false;
            return status;
        } catch (Exception e) {
            return status;
    ...
    public Boolean validateSizeChartContents(List<String> expectedTableHeaders, List<List<String>> expectedTableBody,
        ...
        try {
             ...
             * in the same order
             */
            List<WebElement> tableHeadders = driver.findElements(By.xpath("/html/body/div[2]/div[3]/div/div/table/thead/tr/th"));
            int i = 0;
            for (WebElement tableHead : tableHeadders) {
                if (tableHead.getText().equals(expectedTableHeaders.get(i))) {
            ...
            for (int j=0; j<expectedTableBody.size(); j++) {
                for (int k=0; k<expectedTableBody.get(0).size(); k++) {
                    if (tableData.get(l).getText().equals(expectedTableBody.get(j).get(k))) {
                        l++;
                        // System.out.println(tableData.get(l).getText());
                    } 
                    else {
                        return false;
                    }
    ...
    public Boolean verifyExistenceofSizeDropdown(WebDriver driver) {
        ...
        try {
            ...
            sizeChart.click();
            Thread.sleep(3000);
            WebElement sizeDropdown = driver.findElement(By.xpath("/html/body/div[2]/div[3]/div"));
